...
function CardUsuario({usuario}) {
  ...
  const handleDelete = async () => {
    ...
    swalWithBootstrapButtons.fire({
      ...
      if (result.isConfirmed) {
        deleteUsuarios({id: usuario.id_usuario});
        swalWithBootstrapButtons.fire(
          'Excluido!',
  ...
  const handleEdit = async () => {
    history.push(`edit-address/${usuario.id_usuario}`, {usuario});
  };
  return (
    <Card style={{ width: '18rem' }}>
      <Card.Body>
        <Card.Title>{usuario.nome}</Card.Title>
        <Card.Subtitle className="mb-2 text-muted">{usuario.id_usuario}</Card.Subtitle>
